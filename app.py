...
if all(f is not None for f in files):
    ...
    if all(d is not None for d in dfs):
        ...
        if not group_df.empty:
            ...
            with t2:
                # 1-2) 판매출고(매출원가) 증감 분석
                view2 = group_df[['품목코드', '품목명', '전기동기_판매출고', '당기누적_판매출고', '판매_YoY증감', '당월_판매출고', '전월_판매출고']].sort_values('판매_YoY증감', ascending=False)
                st.dataframe(add_total_row(view2, ['전기동기_판매출고', '당기누적_판매출고', '판매_YoY증감', '당월_판매출고', '전월_판매출고']), use_container_width=True, hide_index=True)

            with t3:
                # 1-1) 생산출고(제조원가) 증감 분석
                view3 = group_df[['품목코드', '품목명', '전기동기_생산출고', '당기누적_생산출고', '생산_YoY증감', '당월_생산출고', '전월_생산출고']].sort_values('생산_YoY증감', ascending=False)
                st.dataframe(add_total_row(view3, ['전기동기_생산출고', '당기누적_생산출고', '생산_YoY증감', '당월_생산출고', '전월_생산출고']), use_container_width=True, hide_index=True)
        else:
        ...
        st.download_button("📥 전체 분석 데이터 다운로드", data=output.getvalue(), file_name=f"Inventory_Analysis_Report_{X}M.xlsx")
