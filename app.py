...
def process_inventory_data(file):
    ...
    try:
        ...
        # 필수 컬럼 존재 여부 확인 (KeyError 방지)
        essential = ['품목코드', '품목명', '단위', '품목계정그룹', '판매출고_금액', '기말재고_금액']
        missing = [c for c in essential if c not in df.columns]
        if missing:
            st.error(f"파일({file.name})에 필수 컬럼이 없습니다: {missing}")
            return None
...
if all(f is not None for f in files):
    ...
    if all(df is not None for df in [df_m_curr, df_m_prev, df_ytd_curr, df_prev_full]):
        ...
        # 상세 테이블 출력 (KeyError 방지를 위해 실제 존재하는 컬럼만 필터링)
        all_display_cols = [
            '품목코드', '품목명', '단위', 
            '당월_판매', '전월_판매', 'MoM_판매증감',
            '당월말_재고', '전기말_재고', '재고증감_vs전기말', 
            '당기YTD_판매', '당기YTD_입고'
        ]
        # 실제 존재하는 컬럼만 선택 (Typos 방지)
        display_cols = [c for c in all_display_cols if c in comp_df.columns]
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            comp_df[display_cols].to_excel(writer, index=False, sheet_name='Analysis')
        st.download_button(label="📥 분석 결과 다운로드", data=output.getvalue(), file_name=f"Analysis_{target_group}.xlsx")
