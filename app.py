...
if all(f is not None for f in files):
    ...
    if all(d is not None for d in dfs):
            ...
            with tabs[0]:
                ...
                if not view1.empty:
                    ...
                    styled_view1 = view1_total.style.format("{:,.0f}", subset=['전기말_재고', '당월말_재고', '재고_증감'])\
                        .map(lambda x: 'color: red' if x > 0 else ('color: blue' if x < 0 else 'color: black'), subset=['재고_증감'])
        ...
        st.download_button("📥 전체 분석 데이터 다운로드", data=output.getvalue(), file_name=f"Accounting_Variance_{X}M.xlsx")
else:
